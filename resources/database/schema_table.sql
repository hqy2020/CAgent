...
CREATE TABLE `t_hotspot_monitor`
(
    `id`                    bigint(20) NOT NULL COMMENT '主键ID',
    `user_id`               bigint(20) NOT NULL COMMENT '用户ID',
    `keyword`               varchar(255) NOT NULL COMMENT '监控关键词',
    `normalized_keyword`    varchar(255) NOT NULL COMMENT '标准化关键词',
    `sources`               varchar(512) NOT NULL COMMENT '数据源列表，逗号分隔',
    `enabled`               tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用',
    `email`                 varchar(255)          DEFAULT NULL COMMENT '通知邮箱',
    `email_enabled`         tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否启用邮件',
    `websocket_enabled`     tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用WebSocket推送',
    `scan_interval_minutes` int(11) NOT NULL DEFAULT '30' COMMENT '扫描间隔（分钟）',
    `relevance_threshold`   decimal(5,4) NOT NULL DEFAULT '0.5500' COMMENT '相关性阈值',
    `credibility_threshold` decimal(5,4) NOT NULL DEFAULT '0.4500' COMMENT '可信度阈值',
    ...
    `last_result_count`     int(11) NOT NULL DEFAULT '0' COMMENT '最近命中数',
    ...
    `update_time`           datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY                     `idx_hotspot_monitor_user` (`user_id`),
    KEY                     `idx_hotspot_monitor_next_run` (`enabled`, `next_run_time`),
    KEY                     `idx_hotspot_monitor_lock` (`lock_until`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='热点监控任务表';

CREATE TABLE `t_hotspot_monitor_event`
(
    `id`                bigint(20) NOT NULL COMMENT '主键ID',
    `monitor_id`        bigint(20) NOT NULL COMMENT '监控任务ID',
    `user_id`           bigint(20) NOT NULL COMMENT '用户ID',
    `keyword`           varchar(255) NOT NULL COMMENT '监控关键词',
    `fingerprint`       varchar(128) NOT NULL COMMENT '事件唯一指纹',
    `title`             varchar(512) NOT NULL COMMENT '标题',
    `summary`           text COMMENT '摘要',
    `url`               varchar(1024) NOT NULL COMMENT '链接',
    `source`            varchar(64)  NOT NULL COMMENT '来源编码',
    `source_label`      varchar(64)  NOT NULL COMMENT '来源名称',
    `published_at`      datetime              DEFAULT NULL COMMENT '发布时间',
    `hot_score`         decimal(12,4)         DEFAULT NULL COMMENT '热度分',
    `relevance_score`   decimal(5,4)          DEFAULT NULL COMMENT '相关性分',
    `credibility_score` decimal(5,4)          DEFAULT NULL COMMENT '可信度分',
    `verdict`           varchar(64)           DEFAULT NULL COMMENT '判定结果',
    `analysis_summary`  varchar(512)          DEFAULT NULL COMMENT 'AI摘要',
    `analysis_reason`   varchar(1024)         DEFAULT NULL COMMENT 'AI说明',
    `author_name`       varchar(255)          DEFAULT NULL COMMENT '作者名',
    `notified_websocket` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否已推送WebSocket',
    `notified_email`    tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否已发送邮件',
    `create_time`       datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`       datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_hotspot_event_fingerprint` (`monitor_id`, `fingerprint`),
    KEY                  `idx_hotspot_event_user` (`user_id`, `create_time`),
    KEY                  `idx_hotspot_event_monitor` (`monitor_id`, `create_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='热点监控事件表';

CREATE TABLE `t_hotspot_monitor_run`
(
    `id`               bigint(20) NOT NULL COMMENT '主键ID',
    `monitor_id`       bigint(20) NOT NULL COMMENT '监控任务ID',
    `user_id`          bigint(20) NOT NULL COMMENT '用户ID',
    `keyword`          varchar(255) NOT NULL COMMENT '监控关键词',
    `status`           varchar(32) NOT NULL COMMENT '执行状态',
    `warning`          varchar(1024)         DEFAULT NULL COMMENT '执行告警',
    `sources`          varchar(512)          DEFAULT NULL COMMENT '本次数据源',
    `expanded_queries` varchar(512)          DEFAULT NULL COMMENT '扩展关键词',
    `fetched_count`    int(11) NOT NULL DEFAULT '0' COMMENT '抓取结果数',
    `qualified_count`  int(11) NOT NULL DEFAULT '0' COMMENT '通过过滤数',
    `new_event_count`  int(11) NOT NULL DEFAULT '0' COMMENT '新增事件数',
    `started_at`       datetime     NOT NULL COMMENT '开始时间',
    `finished_at`      datetime              DEFAULT NULL COMMENT '结束时间',
    `create_time`      datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`      datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY                `idx_hotspot_run_monitor` (`monitor_id`, `started_at`),
    KEY                `idx_hotspot_run_user` (`user_id`, `started_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='热点监控执行记录表';
...
(
    ...
    `tool_id`               varchar(128) NOT NULL COMMENT '工具ID',
    `tool_name`             varchar(255)          DEFAULT NULL COMMENT '工具名称',
    `operation_type`        varchar(16)           DEFAULT NULL COMMENT '操作类型',
    `sensitivity`           varchar(16)           DEFAULT NULL COMMENT '敏感级别',
    `success`               tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否成功',
    `fallback_used`         tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否使用降级',
    `attempt_count`         int(11) NOT NULL DEFAULT '0' COMMENT '尝试次数',
    `retry_count`           int(11) NOT NULL DEFAULT '0' COMMENT '重试次数',
    `circuit_state`         varchar(16)           DEFAULT NULL COMMENT '熔断状态',
    `standard_error_code`   varchar(64)           DEFAULT NULL COMMENT '标准错误码',
    `error_code`            varchar(64)           DEFAULT NULL COMMENT '错误码',
    `error_message`         varchar(1024)         DEFAULT NULL COMMENT '错误信息',
    `request_payload_json`  text COMMENT '脱敏后的请求摘要',
    `response_payload_json` text COMMENT '脱敏后的响应摘要',
    `duration_ms`           bigint(20) NOT NULL DEFAULT '0' COMMENT '耗时毫秒',
    ...
    PRIMARY KEY (`id`),
    KEY                     `idx_mcp_audit_trace` (`trace_id`),
    KEY                     `idx_mcp_audit_tool_time` (`tool_id`, `start_time`),
    KEY                     `idx_mcp_audit_success` (`success`, `start_time`),
    KEY                     `idx_mcp_audit_fallback` (`fallback_used`, `start_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='MCP 工具调用审计表';
