...
(
    ...
    `deleted`     tinyint(4) DEFAULT '0' COMMENT '是否删除 0：正常 1：删除',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='学习模块表';

CREATE TABLE `t_study_chapter`
(
    `id`          bigint(20) NOT NULL COMMENT '主键ID',
    `module_id`   bigint(20) NOT NULL COMMENT '所属模块ID',
    `title`       varchar(256) NOT NULL COMMENT '章节标题',
    `summary`     varchar(512) DEFAULT NULL COMMENT '章节摘要',
    `sort_order`  int(11) NOT NULL DEFAULT '0' COMMENT '排序字段',
    `create_time` datetime     DEFAULT NULL COMMENT '创建时间',
    `update_time` datetime     DEFAULT NULL COMMENT '更新时间',
    `deleted`     tinyint(4) DEFAULT '0' COMMENT '是否删除 0：正常 1：删除',
...
CREATE TABLE `t_study_document`
(
    `id`          bigint(20) NOT NULL COMMENT '主键ID',
    `chapter_id`  bigint(20) NOT NULL COMMENT '所属章节ID',
    `module_id`   bigint(20) NOT NULL COMMENT '所属模块ID（冗余）',
    ...
    KEY           `idx_module_id` (`module_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='学习文档表';

CREATE TABLE `t_interview_category`
(
    `id`          bigint(20) NOT NULL COMMENT '主键ID',
...
(
    ...
    `question`    text         NOT NULL COMMENT '面试题目（Markdown）',
    `answer`      longtext     NOT NULL COMMENT '参考答案（Markdown）',
    `difficulty`  tinyint(4) NOT NULL DEFAULT '1' COMMENT '难度 1：简单 2：中等 3：困难',
