...
const FeedbackSummaryCard = ({
  ...
  summary: DashboardFeedbackSummary | null | undefined;
  loading?: boolean;
  ...
  const metrics = [
    ...
    },
    {
      ...
      helper: `满意 ${formatNumber(summary?.satisfiedCount)}`,
      icon: <ThumbsUp className="h-4 w-4" />,
      tone: "bg-emerald-100 text-emerald-700"
    },
    {
      label: "不满意率",
      value: formatPercent(summary?.dissatisfactionRate),
      helper: `不满意 ${formatNumber(summary?.dissatisfiedCount)}`,
      icon: <ThumbsDown className="h-4 w-4" />,
      tone: "bg-red-100 text-red-700"
    },
    {
      label: "近 24h 不满意",
      value: formatNumber(summary?.recentNegativeCount24h),
      helper: "滚动统计",
  ...
  return (
      <DashCard>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-700">回答满意度</h3>
          <span className="text-[11px] text-slate-400">{timeWindowLabel}</span>
        </div>
          ...
          {metrics.map((item) => (
              <div key={item.label} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={cn("rounded-full p-2", item.tone)}>{item.icon}</span>
                </div>
                {loading && !summary ? (
                    <LoadingBlock className="mt-3 h-6 w-16" />
                ) : (
                    <>
        ...
        <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50 p-3.5">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-xs font-medium text-slate-600">不满意原因分布</p>
              ...
              <div className="space-y-2.5">
                {summary.topReasons.map((item) => (
                    <div key={item.reason} className="space-y-1.5">
                      ...
                      {formatPercent(item.ratio)} · {formatNumber(item.count)}
                    </span>
                      </div>
                      <div className="h-2 rounded-full bg-slate-200">
                        <div
                            className="h-2 rounded-full bg-red-400 transition-[width] duration-500"
                            style={{ width: `${clampPercent(item.ratio)}%` }}
                        />
                      </div>
                    </div>
                ))}
...
const NegativeFeedbackSection = ({
                                   samples,
                                   loading
                                 }: {
  samples: DashboardNegativeSample[];
  loading?: boolean;
}) => {
  return (
      <DashCard>
        <div className="mb-4 flex items-center justify-between">
          <CardTitle>低满意样本</CardTitle>
          <span className="text-[11px] text-slate-400">最近反馈</span>
        </div>
        ...
        {loading ? (
            ...
            <div className="space-y-3">
              {samples.map((sample) => (
                  <div
                      key={`${sample.messageId}-${sample.createdAt}`}
                  ...
                  <span className="rounded-full bg-red-100 px-2.5 py-1 text-[11px] font-medium text-red-600">
                    {sample.reason}
                      ...
                      <span className="text-[11px] text-slate-400">{formatDateTime(sample.createdAt)}</span>
                    </div>

                    <div className="grid gap-3 lg:grid-cols-2">
                      <div className="rounded-xl border border-white/80 bg-white p-3">
                        <p className="mb-1 text-xs font-medium text-slate-500">用户问题</p>
                        <p className="text-sm leading-6 text-slate-700">{sample.question || "暂无问题内容"}</p>
                      </div>
                      <div className="rounded-xl border border-white/80 bg-white p-3">
                        <p className="mb-1 text-xs font-medium text-slate-500">助手回答摘要</p>
                        <p className="text-sm leading-6 text-slate-700">{sample.answer || "暂无回答内容"}</p>
                      </div>
                    </div>
      ...
      <div className="admin-page">
        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_320px]">
          <div className="space-y-5">
            ...
            <NegativeFeedbackSection samples={overview?.negativeSamples ?? []} loading={loading} />
          </div>
          ...
          <aside className="space-y-5 xl:sticky xl:top-4 xl:self-start">
            ...
            />
            <FeedbackSummaryCard
                summary={overview?.feedbackSummary}
                loading={loading}
                timeWindowLabel={WINDOW_LABEL_MAP[timeWindow]}
            />
            <InsightSection
