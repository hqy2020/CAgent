...
package com.openingcloud.ai.ragent.rag.core.retrieve;
...
import com.openingcloud.ai.ragent.rag.dto.KbResult;
...
import static com.openingcloud.ai.ragent.rag.constant.RAGConstant.MULTI_CHANNEL_KEY;
...
public class RetrievalEngine {
    ...
    private static final java.util.regex.Pattern OBSIDIAN_QUERY_HINT = java.util.regex.Pattern.compile(
    ...
    private SubQuestionContext buildSubQuestionContext(SubQuestionIntent intent,
        ...
        // 置信度门控：KB chunk 最高分低于阈值时丢弃全部 KB 结果
        kbResult = applyConfidenceGate(kbResult, intent.subQuestion());
    ...
    private KbResult applyConfidenceGate(KbResult kbResult, String subQuestion) {
        SearchChannelProperties.ConfidenceGate gate = searchChannelProperties.getConfidenceGate();
        if (!gate.isEnabled()) {
            return kbResult;
        }
        if (kbResult.intentChunks() == null || kbResult.intentChunks().isEmpty()) {
            return kbResult;
        }
        float maxScore = kbResult.intentChunks().values().stream()
                .flatMap(List::stream)
                .map(RetrievedChunk::getScore)
                .filter(Objects::nonNull)
                .max(Float::compareTo)
                .orElse(0f);
        if (maxScore < gate.getKbChunkConfidenceThreshold()) {
            log.warn("置信度门控拦截：KB chunk 最高分 {} < 阈值 {}，丢弃 KB 结果。subQuestion={}",
                    maxScore, gate.getKbChunkConfidenceThreshold(), subQuestion);
            return KbResult.empty();
        }
        return kbResult;
    }
    ...
    private boolean isQuestionCompatibleWithTool(String question, String toolId) {
        ...
        return switch (toolId) {
            case WebSearchIntentRules.WEB_NEWS_SEARCH_TOOL_ID -> WebSearchIntentRules.isNewsSearchQuestion(normalized);
            case WebSearchIntentRules.WEB_SEARCH_TOOL_ID -> WebSearchIntentRules.isGeneralWebSearchQuestion(normalized);
            case WebSearchIntentRules.WEB_REALTIME_SEARCH_TOOL_ID -> WebSearchIntentRules.isRealtimeSearchQuestion(normalized);
