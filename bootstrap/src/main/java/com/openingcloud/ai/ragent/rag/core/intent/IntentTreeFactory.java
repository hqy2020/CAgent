...
import static com.openingcloud.ai.ragent.rag.enums.IntentLevel.CATEGORY;
import static com.openingcloud.ai.ragent.rag.enums.IntentLevel.DOMAIN;
import static com.openingcloud.ai.ragent.rag.enums.IntentLevel.TOPIC;
...
public class IntentTreeFactory {
    ...
    private static IntentNode buildKbDomain() {
        ...
        IntentNode projRagent = IntentNode.builder()
                ...
                .description("RAgent 第二大脑系统的架构设计与核心流程")
    ...
    private static IntentNode buildWebSearchDomain() {
        IntentNode domain = IntentNode.builder()
                ...
                .level(DOMAIN)
                .kind(IntentKind.MCP)
                .description("通过多源聚合抓取进行联网检索，覆盖外部实体介绍、公开网页信息与近期新闻热点。")
                .build();

        IntentNode generalSearch = IntentNode.builder()
                .id(WebSearchIntentRules.WEB_SEARCH_GENERAL_NODE_ID)
                .name("通用网页搜索")
                .level(CATEGORY)
                .parentId(domain.getId())
                .kind(IntentKind.MCP)
                .mcpToolId(WebSearchIntentRules.WEB_SEARCH_TOOL_ID)
                .paramPromptTemplate(MCP_PARAMETER_EXTRACT_PROMPT)
                .promptSnippet("当问题是在问外部公司、团队、平台或业务实体“是什么 / 做什么的 / 属于谁”时，优先依据网页证据回答归属关系与职责说明；如果证据不足，直接说明不足，不要把同名技术术语当作答案。")
                .description("联网搜索外部公司、团队、平台、业务和岗位实体的介绍、归属关系与公开说明，优先返回标题、摘要、来源和链接。")
                .examples(List.of(
                        "1688-JAVA-工厂技术 做什么的",
                        "阿里云百炼是什么",
                        "飞书属于哪个公司，主要做什么业务"
        ...
        IntentNode realtimeSearch = IntentNode.builder()
                .id(WebSearchIntentRules.WEB_SEARCH_REALTIME_NODE_ID)
                .name("实时信息搜索")
                .level(CATEGORY)
                .parentId(domain.getId())
                .kind(IntentKind.MCP)
                .mcpToolId(WebSearchIntentRules.WEB_REALTIME_SEARCH_TOOL_ID)
                .paramPromptTemplate(MCP_PARAMETER_EXTRACT_PROMPT)
                .promptSnippet("当问题是在问天气、股价、汇率、票房、比分或其他最新实时事实时，只根据联网结果回答，尽量保留时间和来源，不要回退成静态常识回答。")
                .description("联网搜索天气、行情、汇率、票房、比分、路况等最新实时信息，优先返回摘要、来源和链接。")
                .examples(List.of(
                        "今天上海天气怎么样",
                        "美元兑人民币汇率是多少",
                        "今天比特币价格"
        ...
        IntentNode newsSearch = IntentNode.builder()
                ...
                .parentId(domain.getId())
                .kind(IntentKind.MCP)
                .mcpToolId(WebSearchIntentRules.WEB_NEWS_SEARCH_TOOL_ID)
                .paramPromptTemplate(MCP_PARAMETER_EXTRACT_PROMPT)
                .promptSnippet("当问题是今天 / 最近 / 最新的新闻、热点或动态时，只根据联网结果总结，并尽量保留来源与链接。")
        ...
        domain.setChildren(List.of(generalSearch, realtimeSearch, newsSearch));
        return domain;
    }
