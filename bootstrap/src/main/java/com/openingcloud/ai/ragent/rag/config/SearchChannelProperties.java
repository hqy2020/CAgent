...
public class SearchChannelProperties {
    ...
    public static class QualityFilter {
        ...
        /**
         * 全部被过滤时是否保留最高分 chunk 兜底
         * false（默认）：全部低质量则返回空列表，避免引用不相关内容
         * true：旧行为，保留分数最高的一个
         */
        private boolean keepBestWhenAllFiltered = false;
    }

    /**
     * 置信度门控配置
     * 在 RetrievalEngine 层级对低质量 KB 结果进行二次过滤
     */
    private ConfidenceGate confidenceGate = new ConfidenceGate();
    ...
    public static class ConfidenceGate {
        ...
        /**
         * KB chunk 最高分低于此阈值时，丢弃全部 KB 检索结果
         */
        private float kbChunkConfidenceThreshold = 0.45f;
    }
}
