...
public class RAGConfigProperties {
    ...
    /**
     * 意图低置信度降级阈值
     * 所有子问题的 KB 意图最高分均低于此阈值且无 MCP 匹配时，跳过检索直接走通用 LLM 回答
     */
    @Value("${rag.intent.low-confidence-threshold:0.50}")
    private Double intentLowConfidenceThreshold;

    /**
     * 意图低置信度降级开关
     */
    @Value("${rag.intent.low-confidence-fallback-enabled:true}")
    private Boolean intentLowConfidenceFallbackEnabled;
}
